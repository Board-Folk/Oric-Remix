/*
  Orickeyboard - Oric Keyboard Library
...
// Jump to end if key code is an ignored key
   ...
   if (c >= 160 ) {
      ...
      if (rshift || lshift) {
        //digitalWrite (ANALOG_SW_DATA, LOW);
        //setswitch(OricRSHFT);      (lets not set the right shift, as for some reasons enter gets pressed)
        digitalWrite (ANALOG_SW_DATA, LOW);
        setswitch(OricLSHFT);   
      }
...
// Shift key handling
     if bitRead (flags,6){
      if (c == OricRSHFT) {rshift = true;}
      if (c == OricLSHFT) {lshift = true;}
      }
  
    if bitRead (flags,7){
      digitalWrite(ANALOG_SW_DATA , LOW);
        if (c==OricRSHFT) {rshift = false;}
        if (c==OricLSHFT) {lshift = false;}
        }
...
void Orickeyboard::begin(const OricKeymap_t &map) {
